...
HyperGeneSetCollection_MSF_windows<-function (listOfGeneSetCollections, geneList, hits, pAdjustMethod = "BH",
                                              ...
                                              doGSEA = TRUE, doGSOA = TRUE, entrez2symbs = NULL)
{
  ...
  library(gsubfn)
  require(KEGG.db)
  Res <- analyzeGeneSetCollections_MSF_windows(listOfGeneSetCollections = listOfGeneSetCollections,
  ...
  getKEGGNAMES <- function(kegg.ids) {
    kegg.names <- unlist(as.list(KEGGPATHID2NAME)[str_replace(kegg.ids,
                                                              "hsa", replacement = "")])
    return(kegg.names)
  }
    ...
    for (l in c(1:length(listOfGeneSetCollections))) {
      if (doGSOA) {
        ...
        write.csv(file = paste(names.lists[l], fname,
                               "Hyper.csv", sep = "_"),
                  x = arrange(subset(db,Pvalue <= pValueCutoff), Pvalue), row.names = TRUE)
      }
      if (doGSEA) {
