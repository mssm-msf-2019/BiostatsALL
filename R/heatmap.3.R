#' A function that creates a dataframe to be used as input to create a heatmap
#' @description This function given a numeric matrix returns a data frame with the necessary information to create a heatmap with specifics features
#' @param x numeric matrix 
#' @param Rowv determines if and how the row dendrogram should be computed and reordered. 
#' Either a dendrogram or a vector of values used to reorder the row dendrogram 
#' or NA to suppress any row dendrogram (and reordering) or by default, NULL. It is TRUE by default. 
#' @param Colv determines if and how the column dendrogram should be reordered. 
#' Has the same options as the Rowv argument above and additionally when x is a square matrix,
#' Colv = "Rowv" means that columns should be treated identically to the rows 
#' (and so if there is to be no row dendrogram there will not be a column one either).
#' By defaulf: if (symm) "Rowv" else TRUE.
...
#' Defaults to hclust. Should take as argument a result of distfun and return an object to which as.dendrogram can be applied.
#' @param dendrogram dendrogram and weights for reordering the row and column dendrograms. Defaults to c("both","row", "column", "none").
#' @param symm logical indicating if x should be treated symmetrically; can only be true when x is a square matrix. Defaults on FALSE.
#' @param scale character vector indicating if the values should be centered and scaled in either the row direction or the column direction, or none. 
#' The default is c("none","row", "column").
#' @param na.rm logical indicating whether NA's should be removed. Default is TRUE.
...
#' @param ColSideColors
#' @param RowSideColors
#' @param side.height.fraction=0.3
...
#' @param symkey = max(x < 0, na.rm = TRUE) || symbreaks, densadj = 0.25
#' @param main = NULL, xlab = NULL 
#' @param ylab = NULL, lmat = NULL 
#' @param lhei = NULL, lwid = NULL
#' @param NumColSideColors = 1 
#' @param NumRowSideColors = 1 
